...
def main():
    ...
    parser.add_argument("-proxy", nargs='?', help="Proxy server for the cache")
    ...
    if args.proxy != None:
        try:
            ...
            print("final vars", proxy_host, proxy_port)
            
            if ((proxy_port == None) or (proxy_host == None)):
                raise ValueError

        except ValueError:
            print('Error:  Invalid proxy.  Enter a URL of the form:  http://host:port/file')
            sys.exit(1)


    # Now we try to make a connection to the proxy server.
