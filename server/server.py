...
def prepare_response_message(value):
    ...
    if value == '200':
        ...
        message = message + value + ' File Not\r\n' + date_string + '\r\n'
    return message
...
def main():
    ...
    while(1):
        ...
        # run this command twice to get rid of headers that are not needed
        received_headers = get_line_from_socket(conn)
        received_headers = get_line_from_socket(conn)
        print(received_headers)
        if received_headers != '':
            cache_enabled = True
            header_list = received_headers.split(' ')
            cache_file_modified = header_list[1] + header_list[2]
            print(cache_file_modified)
        else:
            cache_enabled = False
        ...
        # We have the right request and version, so check if file exists.
            ...
            # File exists, so prepare to send it!
                ...
                if cache_enabled == True:
                    ...
                    file_mod_date = modification_date(req_file)
                    
                    # to eval if the file in the server was modified and is newer than the one in the cache
                    if str(file_mod_date) > str(cache_file_modified):
                        #the was modified, resend the file with 200 code
